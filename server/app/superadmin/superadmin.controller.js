...
// @desc    Download platform database backup
// @route   GET /api/superadmin/backup/database
// @access  Private/SuperAdmin
exports.downloadDatabaseBackup = asyncHandler(async (req, res) => {
  const dateStr = new Date().toISOString().replace(/T/, "_").replace(/\..+/, "").replace(/:/g, "-");
  const filename = `globes_db_backup_${dateStr}.zip`;

  res.setHeader("Content-Type", "application/zip");
  res.setHeader("Content-Disposition", `attachment; filename=${filename}`);

  await generateDatabaseBackupStream(res);
});

// @desc    Download platform media backup
// @route   GET /api/superadmin/backup/media
// @access  Private/SuperAdmin
exports.downloadMediaBackup = asyncHandler(async (req, res) => {
  const dateStr = new Date().toISOString().replace(/T/, "_").replace(/\..+/, "").replace(/:/g, "-");
  const filename = `globes_media_backup_${dateStr}.zip`;

  res.setHeader("Content-Type", "application/zip");
  res.setHeader("Content-Disposition", `attachment; filename=${filename}`);

  await generateMediaBackupStream(res);
});
